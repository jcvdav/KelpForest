...
general <- all_points %>% 
  ...
  geom_point(aes(x = longitude, y = latitude, fill = location), size = 2, pch = 21) +
...
north <- all_points %>% 
  ...
  geom_sf(data = World) +
  geom_sf(data = coastline) +
  geom_point(aes(x = longitude, y = latitude), size = 2, pch = 21) +
  # geom_label_repel(aes(x = longitude, y = latitude, label = location)) +
...
cor(a$NMDS1, a$kelp, method = "pearson")

cor(a$NMDS1, a$kelp, method = "spearman")
```
