...
```{r, fig.width = 6, fig.height = 10, fig.cap = "Ecological indices by site for 2011. Sites are arranged latitudinally left - right = nort - south. D = Density (org / transect), J = Evenness, 1 - Lambda = Dominance, S mean = average species richness with and without T0, S total = total number of specie sobserved per site with and without T0."}
Stot <- kelp11 %>% 
  group_by(location, genus_species) %>% 
  summarize(n = sum(abundance, na.rm = T)) %>% 
  filter(n > 0) %>% 
...
ind_2011 <- Stot %>% 
  ...
  mutate(year = 2011) %>% 
  mutate(group = fct_relevel(group, c("D'", "J & 1 - Lambda", "Shannon")))
...
```{r, fig.height = 10, fig.width = 10, fig.cap = "Ordination analysis for 17 kelp forests evaluated in 2011. Panels A - C show all transects colored by sites (A), and habitat (B). Panel C shows the ordination analysis for mean densities per site and zone but only for bottom transects.."}
plot_grid(p1, p2, p3, ncol = 2, labels = "AUTO")
```
...
```{r, fig.width = 6, fig.height = 8, fig.cap = "Comparision of ecological indices by site for 2011 and 2013. Sites are arranged latitudinally left - right = nort - south. D = Density (org / transect), J = Evenness, 1 - Lambda = Dominance, S mean = average species richness with and without T0, S total = total number of specie sobserved per site with and without T0."}
Stot <- kelp13 %>% 
  group_by(location, genus_species) %>% 
  summarize(n = sum(abundance, na.rm = T)) %>% 
  filter(n > 0) %>% 
...
ggplot(ind_2013, aes(x = Location, y = Value, shape = year)) +
  ...
  geom_point(aes(color = Indicator), size = 2.5, position = position_dodge(width = 0.5)) + 
  facet_wrap(~group, scales = "free", ncol = 3) +
  coord_flip() +
  theme(strip.background = element_blank(),
        legend.justification = c(0, 0),
