...
# Cambios al 28/05/2018

- Renombrar Sitios, Localidades, Zonas, Niveles, Habitat...

    - Sitio es el manto, que se divide en Zonas N y S y tiene habitats F y MA.

- En la figura 4, usar solamente 3 nMDS:
  
    - Todos los transectos, coloreados por sitio
  
    - Todos los transectos, coloreados por habitat
  
    - Promedios de densidad por sitio y zona, solamente transectos fondo
  
    - Seria mejor cambiar a otro esquema de colores?

- Generar tabla 2
    
    - Densidades por especie y sitio (con desviaciones estandar)
    
    - Solamente ano 2011
    
    - No queremos una igual para 2013? O por lo menos un listado de presencia ausencia para todos los sitios / todos los años?
...
- Comparar 2011 - 2013
    ...
    - Modificar nMDS para tener un punto por sitio
  
Notas: 

La transición por densidades se ve medio rara. Al hacerlo por IBV se "normalizan" las densidades para saber ponerla densidad en contexto con toda la comunidad. Posiblemente una forma itnermedia sea densidad o abundancia relativa?
...
Stot %>% 
  ...
  knitr::kable(escape = F,
               col.names = c("Site", "S0", "S", "$\\bar{S0}$", "$\\bar{S}$", "D", "H'", "J'", "$1 - \\lambda$"))
```

### Figura

```{r, fig.width = 8, fig.height = 5, fig.cap = "Ecological indices by site for 2011. Sites are arranged latitudinally left - right = nort - south."}
...
S_2011 <- specpool(x = S_matrix, pool = S_groups$location, smallsample = T) %>% 
  as.data.frame() %>% 
...
p3 <- plot_data %>% 
  ...
  coord_equal() +
  scale_fill_manual(values = colorRamps::matlab.like(17), guide=FALSE) +
  annotate(geom = "text", x = 0, y = 0.5, label = stress)+
  scale_shape_manual(values = c(21, 24), name = "Zone") +
  coord_equal(ratio = 1.2) +
  theme(legend.text = element_text(size = 8),
        legend.title = element_text(size = 10))
```

```{r, fig.height = 10, fig.width = 5, fig.cap = "Ordination analysis for 17 kelp forests evaluated in 2011. Panels A - C show all transects colored by sites (A), and habitat (B). Panel C shows the ordination analysis for mean densities per site and zone but only for bottom transects."}
...
kelp11 %>% 
  ...
  knitr::kable(escape = F, caption = "Density (org / transect) by species and site. Sites are arranged latitudinally left - right = north - south.")
...
Stot_wo0 %>% 
  ...
  select(-c(latitude, longitude)) %>% 
  knitr::kable(escape = F,
               col.names = c("Location", "S", "D", "H'", "J'", "$1 - \\lambda$"))
```

### Figura

```{r, fig.width = 8, fig.height = 4, fig.cap = "Comparision of ecological indices by site for 2011 (blue) and 2013 (red). Sites are arranged latitudinally left - right = nort - south."}


S_data <- kelp %>% 
  filter(transect > 0,
         location %in% c("ASA", "BMA", "ERE", "ERO", "ISME", "ITSP", "RET", "SMI", "SSI"),
         level == "Bottom") %>% 
...
S_2013 <- specpool(x = S_matrix, pool = S_groups$group, smallsample = T) %>% 
  as.data.frame() %>% 
...
cowplot::plot_grid(index_2011, index_2013, ncol = 1)
```
...
anosim_c_cedros <- kelp %>% 
  filter(location %in% c("ASA", "BMA", "ERO", "ISME", "ITSP", "RET", "SMI", "SSI"),
         level == "Bottom") %>% 
...
cbind(data_c_cedros_groups, scores(mds)) %>% 
  ...
  annotate(geom = "text", x = -0.2, y = 0.2, label = stress)
```

## nMDS fondo y media agua

```{r, fig.height = 8.5, fig.width = 6, fig.cap = "nMDS for sites by bottom (A), and midwater (B)."}
anosim_c_cedros <- kelp %>% 
...
# Reciclamos el codigo y lo hacemos ahora para media agua
anosim_c_cedros <- kelp %>% 
  filter(location %in% c("ASA", "BMA", "ERE", "ERO", "ISME", "ITSP", "RET", "SMI", "SSI"),
         !level == "Bottom") %>% 
  group_by(year, location, site, level, transect, genus_species) %>% 
