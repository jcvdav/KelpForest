...
all_points <- group_by(kelp, location, latitude, longitude) %>%
  summarize(ln = n()) %>%
  ungroup() %>% 
  transform(location = reorder(location, -latitude))
...
general <- all_points %>% 
  ...
  geom_point(aes(x = longitude, y = latitude), size = 2, pch = 21, fill = "steelblue") +
  # geom_label_repel(aes(x = longitude, y = latitude, label = location)) +
  scale_x_continuous(limits = c(-120, -108)) +
  scale_y_continuous(limits = c(22, 36)) +
  theme_bw()
...
north <- all_points %>% 
  ...
  geom_point(aes(x = longitude, y = latitude), size = 2, pch = 21, fill = "steelblue") +
  # geom_label_repel(aes(x = longitude, y = latitude, label = location)) +
  scale_x_continuous(limits = c(-117.5, -116)) +
  scale_y_continuous(limits = c(31.5, 32.5)) +
  theme_bw()
...
center <- all_points %>% 
  ...
  geom_point(aes(x = longitude, y = latitude), size = 2, pch = 21, fill = "steelblue") +
  # geom_label_repel(aes(x = longitude, y = latitude, label = location)) +
  scale_x_continuous(limits = c(-117,-115)) +
  scale_y_continuous(limits = c(29.5, 31.5)) +
  theme_bw()
...
south <- all_points %>% 
  ...
  geom_point(aes(x = longitude, y = latitude), size = 2, pch = 21, fill = "steelblue") +
  # geom_label_repel(aes(x = longitude, y = latitude, label = location)) +
  scale_x_continuous(limits = c(-116,-114)) +
  scale_y_continuous(limits = c(27.5, 30)) +
  theme_bw()

right <- plot_grid(north, center, south, ncol = 1)
plot_grid(general, right)
```
...
ggplot(data = mdsT, aes(x = NMDS1, y = NMDS2, fill = temp, z = temp)) +
  geom_point(size = 4, shape = 21) + geom_contour() +
  geom_line(aes(group = location), linetype = "dashed") +
  theme(aspect.ratio = 1)
...
mdsT %>% 
  ...
  ggplot(aes(x = NMDS1, y = NMDS2)) +
  geom_line(aes(group = location), linetype = "dashed") +
  geom_point(aes(color = location, shape = year, size = temp)) +
  scale_color_brewer(palette = "Paired", name = "Location") +
  scale_shape_discrete(name = "Year") +
  annotate(geom = "text", x = 0, y = 0.3, label = stress) +
  theme(aspect.ratio = 1,
...
kelp %>% 
  ...
  ggplot(aes(x = location, y = kelp_density, color = as.character(year))) +
  geom_point() + 
  geom_errorbar(aes(ymin = kelp_density - sd, ymax = kelp_density + sd), width = 0.2) +
  coord_flip()
```
...
a <- cbind(data_c_cedros_groups, scores(mds)) %>% 
...
a %>% 
  ...
  ggplot(aes(x = NMDS1, y = NMDS2)) +
  geom_line(aes(group = location), linetype = "dashed") +
  geom_point(aes(color = location, shape = year, size = kelp)) +
  geom_contour(aes(z = kelp)) +
  scale_color_brewer(palette = "Paired", name = "Location") +
  scale_shape_discrete(name = "Year") +
  annotate(geom = "text", x = 0, y = 0.3, label = stress) +
  theme(aspect.ratio = 1,
...
plot(NMDS1 ~ kelp, data = a)

r <- cor(a$NMDS1, a$kelp, method = "pearson")
tscore <- r * sqrt((18 - 2) / (1 - r^2))
p <- 2*pt(tscore, df = 16, lower.tail=FALSE)
```
...
data_test2_groups <- data_test2 %>% 
  select(location, site, level, transect) %>% 
  mutate(loc_site = paste(location, site, sep = "-"))
...
kelp_2011 <- kelp11 %>% 
  filter(transect > 0) %>% 
  group_by(location, site, transect, kelp_density) %>% 
  count() %>% 
  ungroup()
...
plot_data2 <- cbind(data_test2_groups, scores(mds)) %>% 
  left_join(kelp_2011, by = c("location", "site", "transect"))

plot_data2 %>% 
  ggplot(aes(x = NMDS1, y = NMDS2, sitio = site)) +
  geom_point(aes(fill = location, size = kelp_density), alpha = 0.8, shape = 21) +
  coord_equal() +
  scale_fill_manual(values = colorRamps::matlab.like(17), guide = guide_legend(ncol = 2, title = "Location")) +
  annotate(geom = "text", x = 0, y = 0.5, label = stress)+
  theme(legend.text = element_text(size = 8),
...
cor(plot_data2$NMDS1, plot_data2$kelp_density, use = "pairwise.complete.obs")
```
...
data_test2_samples <- data_test2 %>% 
  select(-c(location, site)) %>% 
  vegdist(method = "bray")

kelp_2011 <- kelp11 %>% 
  filter(transect > 0) %>% 
  group_by(location, site) %>% 
  summarize(kelp_density = mean(kelp_density, na.rm = T)) %>% 
  ungroup()
...
plot_data2 <- cbind(data_test2_groups, scores(mds)) %>% 
  left_join(kelp_2011, by = c("location", "site"))
