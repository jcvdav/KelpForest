...
```{r}
taxa <- load_taxa() %>% 
  select(GenusSpecies = Species, Class, Order, Family, Genus) %>% 
  filter(GenusSpecies %in% sp_list) %>% 
  as_tibble()
```

```{r}
sp_info <- species(species_list = sp_list, fields = c("Species",
                                                      "BodyShapeI",
                                                      "DemersPelag",
                                                      "PriceCateg",
...
```{r}
write_csv(data_final, file = here("data","Peces_KelpForest_2011-2013.csv"))
```
